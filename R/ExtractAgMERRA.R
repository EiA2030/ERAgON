...
ExtractAgMERRA<-function(DATA,
  ...
  if(file.exists(paste0(Save_Dir,"AgMERRA.RData"))){
    ...
    X<-miceadds::load.RData2(file=paste0(Save_Dir,"AgMERRA.RData"))
    YStart<-Years[1]
    YEnd<-Years[2]
  ...
  for (i in 1:length(AgMERRA_list)){
    ...
    flush.console()
    if (i==1){
      AgMERRA<-miceadds::load.Rdata2(file=paste0(temp_dir,AgMERRA_list[i]))
    }else{
      Z<-miceadds::load.Rdata2(file=paste0(temp_dir,AgMERRA_list[i]))
