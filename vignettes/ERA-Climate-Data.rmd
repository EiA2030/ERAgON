...
```{r Add PET, echo =T, eval=F}
    POWER[,ETo:=ERAgON::PETcalc(Tmin=Temp.Min,
                      Tmax=Temp.Max,
                      SRad=SRad,
                      Wind=WindSpeed,
                      Rain=Rain,
...
When harvest dates are not reported in the studies that comprise ERA we can substitute season length values from the EcoCrop dataset. First we need to merge EcoCrop data with `ERAgON::AddEcoCrop` function.
...
ERA.Yields<-ERAgON::EstSLenData(DATA=ERA.Yields)
...
ERA.Yields<-ERAgON::EstPDayRain(Data=ERA.Yields, 
                    ID=ID, 
...
Climate<-ERAgON::CalcClimate(DATA=ERA.Yields,
                ID=ID,
...
Climate<-ERAgON::Climate.Vignette
```
